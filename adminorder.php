    ...
    <nav class="navbar navbar-dark fixed-top bg-custom flex-md-nowrap p-0 shadow">
     ...
     <div class="bg-light border-right sidenav" id="sidebar-wrapper">
      <div class="list-group list-group-flush">
            ...
            <a href="adminorders.php" class="menu-item"><span class="icon" ><i class="fas fa-user"></i></span>Orders</a>
...
while ( $row = $result->fetch(PDO::FETCH_ASSOC)){
   ...
   $second=$inventory->getInventoryName($row["inventory_id"]);
  while ( $row2 = $second->fetch(PDO::FETCH_ASSOC)){
     $Inventoryname= $row2['inventory_name']; 
  };

  $third=$customer->getCustomerEmail($row["customer_id"]);
  while ( $row3 = $third->fetch(PDO::FETCH_ASSOC)){
     $Customername= $row3['email']; 
  };
      
          ?>		

            <tr id="contacting2" class="<?php echo $row["order_no"];  ?>">
                <form>
                    ...
                    <td >
                    <a type="submit" href=" Backend/config/update.php?table=orders&id=<?php echo $row['order_no'];?>&productname=<?php echo $Inventoryname; ?>&customername=<?php echo $Customername;?>&qty=<?php echo $row["qty_ordered"]; ?>"  >
                    <i class="fa fa-fw fa-edit"></i>
                        </a> |
                </form>     
                <a href="Backend/config/delete.php?table=orders&id=<?php echo $row['order_no'];?>" class="text-danger" onClick="return confirm('Are you sure you want to delete this user?');"><i class="fa fa-fw fa-trash"></i></a> 
            </td>
